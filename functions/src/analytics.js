...
async function syncAnalytics(issueId) {
  await db.runTransaction(async (tx) => {
    const source = db.doc(`issues/${issueId}`),
      ledger = db.doc(`analyticsContributions/${issueId}`),
      summaryRef = db.doc('analytics/community');
    ...
    const ownerStats = await Promise.all(
      owners.map((uid) => tx.get(db.doc(`users/${uid}/achievements/summary`))),
    );
    ...
    metrics.byDay = Object.fromEntries(
      Object.entries(metrics.byDay || {}).filter(([day]) => day >= cutoff),
    );
    tx.set(summaryRef, { ...metrics, updatedAt: FieldValue.serverTimestamp() });
    owners.forEach((uid, index) => {
      const stats = ownerStats[index].data() || {};
      const reports =
        (stats.reports || 0) + Number(next?.ownerId === uid) - Number(old?.ownerId === uid);
      const resolved =
        (stats.resolved || 0) +
        Number(next?.ownerId === uid && next.status === 'Resolved') -
        Number(old?.ownerId === uid && old.status === 'Resolved');
      tx.set(ownerStats[index].ref, {
        ...achievements(reports, resolved),
        updatedAt: FieldValue.serverTimestamp(),
      });
    });
    if (next) tx.set(ledger, { value: next });
    else tx.delete(ledger);
  });
}
