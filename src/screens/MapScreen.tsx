...
export function MapScreen() {
  ...
  const locate = async () => {
    ...
    setError('');
    try {
      setCenter(await currentLocation());
    } catch (e) {
  ...
  const issues = useIssueStore((s) => s.issues).filter(
    (i) =>
      ...
      `${i.title} ${i.address} ${reportReference(i.id)}`
        .toLowerCase()
  ...
  return (
    <Screen>
      ...
      <Field
        ...
        placeholder="Issue, street, or reference number"
      />
      ...
      <Text style={styles.caption}>
        {pinned.length} reports with a map pin · {issues.length - pinned.length} address-only
        reports. Tap a pin or choose a report below.
      </Text>
      {selected && (
      ...
      <SectionHeader
        ...
        subtitle={`${issues.length} matching community reports`}
      />
      {issues.map((i) => (
        <Pressable
        ...
        >
          <View style={{ flex: 1, gap: 7 }}>
            ...
            <Text style={styles.caption}>
              {i.latitude == null ? 'Address only · No GPS pin' : 'GPS location attached'}
            </Text>
          </View>
