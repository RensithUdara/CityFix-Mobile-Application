...
const source = ts.transpileModule(
  fs.readFileSync('src/services/uploadPreparedPhoto.native.ts', 'utf8'),
...
function load({ upload, readError = false }) {
  ...
  class NativeRequest {
    ...
    open(method, uri) {
      ...
      assert.equal(uri, 'file:///normalized.jpg');
    }
    send() {
  ...
  vm.runInNewContext(source, {
    ...
    require: () => ({ uploadBytes: upload }),
    Blob: class {
      constructor() {
        throw new Error('ArrayBuffer Blob construction is unsupported');
      }
    },
  });
...
test('native upload passes the original native Blob with explicit JPEG metadata and releases it', async () => {
  const fixture = load({
    upload: async (target, data, metadata) => {
      assert.equal(target, 'target');
      assert.equal(data, fixture.blob);
